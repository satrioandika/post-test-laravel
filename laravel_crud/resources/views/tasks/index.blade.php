        ...
        <tbody class="text-center">
          ...
          <tr>
            ...
            <td style="text-align: center;">
              <a href="{{ route('tasks.edit', ['id' => $tugas->id]) }}" class="btn btn-warning btn-sm"><i class="fa fa-edit"></i> Edit</a>
              <a href="#" class="btn btn-sm btn-danger" onclick="
              event.preventDefault();
              if(confirm('Apakah Anda yakin ingin menghapus data ini?')) {
                document.getElementById('delete-row-{{ $tugas->id }}').submit();
              }"><i class="fa fa-trash"></i> Hapus</a>
              <form method="POST" id="delete-row-{{ $tugas->id }}" action="{{ route('tasks.destroy', ['id' => $tugas->id]) }}" style="display: none;">
                @method('DELETE')
                @csrf
              </form>
            </td>
